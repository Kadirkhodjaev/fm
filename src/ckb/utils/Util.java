...
public class Util {
  ...
  private static final String dig1[][] = {{"одна", "две", "три", "четыре", "пять", "шесть", "семь", "восемь", "девять"}, {"один", "два"}};
  ...
  public static String inwords(double money) {
    ...
    if (num<1000000000000000l) {
      ...
      return "error: слишком большая сумма " + skop + " " + leword[0][iw];
  }
  ...
  public static boolean getMsg(HttpServletRequest request, Model model){
    ...
    if(str != null) {
      if(request.getParameter("msgState").equals("0")) // Ощибка
        model.addAttribute("errorMsg", request.getParameter("msgCode"));
      else if (request.getParameter("msgState").equals("1")) // Успешно
        model.addAttribute("messageCode", request.getParameter("msgCode"));
      else if (request.getParameter("msgState").equals("3")) { // Успешно
        model.addAttribute("messageCode", request.getParameter("msgCode"));
        return true;
      }
    }
