...
var MonthYearColor = "#444142"; //Font Color of Month and Year in Calendar header.
var WeekHeadColor = "#656865"; //var WeekHeadColor="#18861B";//Background Color in Week header.
var SundayColor = "#BDCAE4"; //var SundayColor="#C0F64F";//Background color of Sunday.
var SaturdayColor = "#E7E7E7"; //Background color of Saturday.
var WeekDayColor = "#E7E7E7"; //Background color of weekdays.
var FontColor = "blue"; //color of font in Calendar day cell.
var TodayColor = "#BBB4A7"; //var TodayColor="#FFFF33";//Background color of today.
var SelDateColor = "#9FA09F"; //var SelDateColor = "#8DD53C";//Backgrond color of selected date in textbox.
var YrSelColor = "#444142"; //color of font of Year selector.
var MthSelColor = "#444142"; //color of font of Month selector if "MonthSelector" is "arrow".
var HoverColor = "#9FA09F"; //color when mouse move over.
...
var imageFilesPath = "/res/datepicker/images/";
...
var WeekDayName1 = ["Вск","Пнд","Втр","Срд","Чтв","Пят","Суб"];
var WeekDayName2 = ["Пнд","Втр","Срд","Чтв","Пят","Суб","Вск"];
...
function initDates(){
  $('.datepicker').each(function () {
    ...
    if (!elem.hasAttribute('placeholder')) {
      input.attr("placeholder", "dd.mm.yyyy").on("dblclick", function () {
        getCalendar(this, 'ddMMyyyy', 'arrow');
      }).attr("readonly", "readonly").css("float", "left");
      var sp = $("<div>");
      sp.attr("class", "dateImg");
      sp.on("click", function () {
        getCalendar(elem, 'ddMMyyyy', 'arrow');
      });
      input.after(sp);
    }
  });
...
function RenderCssCal(bNewCal)
{
	...
	winCalData = "<span style='cursor:auto; display: block;'>";
	vCalHeader = "<table style='display: block; background-color:"+CalBgColor+";width:200px;padding:0;margin:5px auto 5px auto'><tbody>";
	...
	if (Cal.ShowTime === true)
	{
		...
		vCalTime += "<td width='22px'><input type='text' name='hour' maxlength=2 size=1 style=\"WIDTH:22px;text-align:center;margin-bottom:1px !important;height:18px !important\" value=" + showHour + " onkeyup=\"javascript:Cal.SetHour(this.value)\">";
		vCalTime += "</td><td style='font-weight:bold;text-align:center;'>:</td><td width='22px'>";
		vCalTime += "<input type='text' name='minute' maxlength=2 size=1 style=\"WIDTH: 22px;text-align:center;margin-bottom:1px !important;height:18px !important\" value=" + Cal.Minutes + " onkeyup=\"javascript:Cal.SetMinute(this.value)\">";
		...
		vCalTime += "<tr><td colspan='8' style=\"text-align:center;\"><input style='width:75px;font-size:12px;' onClick='javascript:closewin(\"" + Cal.Ctrl + "\");'  type=\"button\" value=\"Сохранить\">&nbsp;<input style='width:65px;font-size:12px;' onClick='javascript: winCal.style.visibility = \"hidden\"' type=\"button\" value=\"Закрыть\"></td></tr>";
	}
...
function getCalendar(pCtrl, pFormat, pScroller, pShowTime, pTimeMode, pShowSeconds, pEnableDateMode)
{
	...
	exDateTime = pCtrl.value; //Existing Date Time value in textbox.
...
function checkDate(elem, msg, isTime) {
  if(elem.value != "") {
    re = /,/g;
    elem.value = (elem.value).replace(re, ".");

    var filter = isTime
      ? /(^(((0|1|2)\d)|(30|31){1})(01|02|03|04|05|06|07|08|09|10|11|12){1}\d{4}(\s\d\d\:\d\d)?$)|(^$)/gi
      : /(^(((0|1|2){1}\d{1})|(30|31){1})(01|02|03|04|05|06|07|08|09|10|11|12){1}\d{4}$)|(^$)/;;
    if (filter.test(elem.value) && event.srcElement.value!="") {
      var t = elem.value;
      elem.value=t.substring(0,2)+"."+t.substring(2,4)+"."+t.substring(4,8);
    }
    //if (elem.value.substring(0,2)=='00') {alert(msg); elem.value=""; elem.focus(); return false;}
    filter = isTime
      ? /(^(((0|1|2){1}\d{1})|(30|31){1})\.(01|02|03|04|05|06|07|08|09|10|11|12){1}\.\d{4}(\s\d\d\:\d\d)?$)|(^$)/
      : /(^(((0|1|2){1}\d{1})|(30|31){1})\.(01|02|03|04|05|06|07|08|09|10|11|12){1}\.\d{4}$)|(^$)/;
    if (!filter.test(elem.value)) {
      alert(msg + (isTime ? 'DD.MM.YYYY HH24:MI' : 'DD.MM.YYYY'));
      elem.value = "";
      return false;
    }
    return true;
...
function getTodayDate(time){
  ...
  var d = new Date();
  var date =  checkDateTime(d.getDate()) + "." + checkDateTime(parseInt(d.getMonth(), 10) + 1) + "." + d.getFullYear();
  if(time){
    date = date + " " + checkDateTime(d.getHours()) + ":" + checkDateTime(d.getMinutes());
  }
  return date;
}