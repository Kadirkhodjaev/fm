...
<script>
  function addAmb() {
    $('#pager').load('/core/ambv2/group/save.s');
  }
</script>
<style>
    ...
    <div class="table-responsive">
      <table class="miniGrid table table-striped table-bordered">
        ...
        <c:forEach items="${groups}" var="g" varStatus="loop">
          <tr ondblclick="$('#pager').load('/core/ambv2/group/save.s?id=${g.id}');return false;">
            <td style="vertical-align: middle" class="center">${loop.index + 1}</td>
            <td style="vertical-align: middle">
              <a href="#" onclick="$('#pager').load('/core/ambv2/group/save.s?id=${g.id}');return false;">${g.name}</a>
            </td>
            <td class="center">
                ${g.partnerProc}
            </td>
