  ...
  <div class="" style="margin-left:-1px; width: 100% !important;">
    ...
    <form id="amb-form">
      ...
      <table width="100%" class="table-bordered">
        <tr>
          <td class="center bold" rowspan="2">Название исследования</td>
          <td class="center bold" colspan="2">Результат</td>
        </tr>
        <tr>
          <td class="center bold">Количественный</td>
          <td class="center bold">Относительный<br>Lg(X/ОБМ)</td>
        </tr>
        <c:forEach items="${fields}" var="a" varStatus="loop">
          <c:if test="${loop.index % 2 == 0}">
            <tr>
              <td style="padding:5px;text-align:right">${a.name}</td>
              ...
              <td style="padding:5px">${fields[loop.index + 1].html}</td>
            </tr>
          </c:if>
          <c:if test="${loop.index == 2}">
            <tr>
              <td colspan="4" style="padding:5px;text-align: center;font-weight: bold">НОРМОФЛОРА</td>
            </tr>
          </c:if>
          <c:if test="${loop.index == 5}">
            <tr>
              <td colspan="4" style="padding:5px;text-align: center;font-weight: bold">ОБЛИГАТНО-АНАЭРОБНЫЕ МИКРООРГАНИЗМЫ</td>
            </tr>
          </c:if>
          <c:if test="${loop.index == 7}">
            <tr>
              <td colspan="4" style="padding:5px;text-align: center;font-weight: bold">ДРОЖЖЕПОДОБНЫЕ ГРИБЫ</td>
            </tr>
          </c:if>
          <c:if test="${loop.index == 9}">
            <tr>
              <td colspan="4" style="padding:5px;text-align: center;font-weight: bold">МИКОПЛАЗМЫ</td>
            </tr>
          </c:if>
          <c:if test="${loop.index == 13}">
            <tr>
              <td colspan="4" style="padding:5px;text-align: center;font-weight: bold">ПАТОГЕННЫЕ МИКРООРГАНИЗМЫ</td>
            </tr>
          </c:if>
        </c:forEach>
      </table>
