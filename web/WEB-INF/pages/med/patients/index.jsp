...
<script>
  ...
  function addEditPatient(id) {
    alert(${roleId});
    if(${roleId != 5 && roleId != 9 && roleId != 8 && roleId != 6 && roleId != 7 && roleId != 11 && curUrl != '/patients/archive.s'})
    ...
    <table class="table table-striped table-bordered table-hover grid dataTable hand">
      <thead>
        <tr>
          ...
          <th class="sorting" id="thregdate" onclick="setSort('Date_Begin', null)">Дата рег.</th>
          <th>Номер И/Б</th>
          <th>Отд./Пал.</th>
          <th class="sorting" id="thcat" onclick="setSort('Cat_Id', 'cat')">Категория</th>
          <th class="sorting" id="thmetka" onclick="setSort('metka_id', 'metka')">Метка</th>
          <th class="sorting" id="thlv" onclick="setSort('lv_id', 'lv')">ФИО ЛВ</th>
        </tr>
      </thead>
