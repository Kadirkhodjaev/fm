  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title" id="myModalLabel">Реквизиты единицы измерения</h4>
      </div>
      <div class="modal-body">
        <form id="addEditForm" name="addEditForm">
          <input type="hidden" name="id" value="" />
          <input type="hidden" name="code" value="measure" />
          <table class="table table-bordered">
            <tr>
              <td class="right bold">Наименование*:</td>
              <td>
                <input type="text" id="measure-name" class="form-control" name="name" value=""/>
              </td>
            </tr>
            <tr>
              <td class="right bold">Ед. изм. для прихода?:</td>
              <td class="left">
                <input type="checkbox" checked name="income" value="Y"/>
              </td>
            </tr>
            <tr>
    ...
    $.ajax({
      ...
      success: function (res) {
        if (res.success) {
          ...
          $('*[name=income]').prop('checked', res.income);
        } else {
          alert(res.msg);
        }
      }
    ...
    $.ajax({
      ...
      success: function (res) {
        if (res.success) {
          ...
          $('#close-modal').click();
          setPage('/head_nurse/dicts/measures.s');
        } else {
          alert(res.msg);
        }
      }
